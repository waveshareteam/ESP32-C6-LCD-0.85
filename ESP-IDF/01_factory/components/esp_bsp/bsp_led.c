    ...
    while (1)
    {
        if (xSemaphoreTake(led_effect_semaphore, portMAX_DELAY) == pdTRUE)
        {
            while (led_effect_running)
            {
                ...
                switch (color_idx)
                {
                    case 0: bsp_led_set_all(0, 255, 0, led_brightness); break;  
                    case 1: bsp_led_set_all(0, 0, 255, led_brightness); break;  
                    case 2: bsp_led_set_all(255, 255, 0, led_brightness); break;  
                    case 3: bsp_led_set_all(255, 0, 255, led_brightness); break;  
                }
                vTaskDelay(pdMS_TO_TICKS(300));
            }
            bsp_led_clear();
