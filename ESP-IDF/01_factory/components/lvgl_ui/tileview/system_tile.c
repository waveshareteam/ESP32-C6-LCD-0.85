...
static void system_time_cb(lv_timer_t *timer)
{
    ...
    sprintf(str, "BAT %.2f V", bat_voltage);
...
void system_tile_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(list, lv_pct(120), lv_pct(100));
    // lv_obj_center(list);
    ...
    lv_obj_t *list_item;
    list_item = lv_list_add_btn(list, NULL, NULL);
    ...
    // psram_size = 0;//ESP32-C6FH8 does not have PSRAM.
    // lv_label_set_text_fmt(label_psram, "PSRAM %d MB", (int)(psram_size / 1024 / 1024));


    list_item = lv_list_add_btn(list, NULL, NULL);
    lv_obj_t *label_flash = lv_label_create(list_item);
    esp_flash_get_size(NULL, &flash_size);
    lv_label_set_text_fmt(label_flash, "Flash %d MB", (int)(flash_size / 1024 / 1024));


    list_item = lv_list_add_btn(list, NULL, NULL);
    lv_obj_t *label_sd = lv_label_create(list_item);
    sdcard_size = bsp_sdcard_get_size();
    lv_label_set_text_fmt(label_sd, "SD %d MB", (int)(sdcard_size / 1024 / 1024));
